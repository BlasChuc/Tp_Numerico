...
def runge_kutta_4_orden_superior(f_u, t0, y0, u0, tf, h):
    """
    ...
    for i in range(1, n):
        # 1era iteración
        k1 = u[i-1]
        m1 = f_u(t[i-1], y[i-1], u0)

        # 2da iteración
        k2 = u[i-1] + (h * k1) / 2
        m2 = f_u(t[i-1] + h/2, y[i-1] + (h * k1)/2, u[i-1] +(h * m1)/2)
        
        # 3ra iteración
        k3 = u[i-1] + (h * k2) / 2
        m3 = f_u(t[i-1] + h/2, y[i-1] + (h * k2)/2, u[i-1] +(h * m2)/2)

        # 4ta iteración
        k4 = u[i-1] + (h * k3) / 2
        m4 = f_u(t[i-1] + h, y[i-1] + (h * k3), u[i-1] + (h * m3))
...
def ecuacion_curvas(r, theta, max_G):
    if max_G > ACELERACION_MAX:
        return 0
    
    return (- max_G * np.sin(theta)) / r
